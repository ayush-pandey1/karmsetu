...
const SideNav = () => {
  ...
  useEffect(() => {
    if (userId) {
      ...
      const handleReceiveApplication = (data) => {
        console.log("Live application notification received:", data);
        showClientApplicationPopup(data);
      };

      socketInstance.on("get-users", handleGetUsers);
      socketInstance.on("recieve-message", handleReceiveMessage);
      socketInstance.on("recieve-application", handleReceiveApplication);

      return () => {
        socketInstance.off("get-users", handleGetUsers);
        socketInstance.off("recieve-message", handleReceiveMessage);
        socketInstance.off("recieve-application", handleReceiveApplication);
      };
    }
  }, [userId, dispatch]);
